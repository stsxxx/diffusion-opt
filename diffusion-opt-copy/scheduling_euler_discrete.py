...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def scale_model_input_bs(self, sample: torch.Tensor, timesteps: Union[List[float], torch.Tensor]) -> torch.Tensor:
        """
        Ensures interchangeability with schedulers that need to scale the denoising model input depending on the
        current timestep. Scales the denoising model input by `(sigma**2 + 1) ** 0.5` to match the Euler algorithm.
        ...
        # Initialize the step index if not done already, based on the provided timesteps
        # if self.step_index is None:
        self._init_step_index_bs(timesteps)

        # Retrieve the sigmas for each timestep in the batch
        sigmas = self.sigmas[self.step_index].to(sample.dtype)


        # Reshape sigmas to match the batch dimension (batch_size, 1, 1, ...)
        while len(sigmas.shape) < len(sample.shape):
            sigmas = sigmas.unsqueeze(-1)
        sigmas = sigmas.repeat(2, 1, 1, 1)  
        # Scale each sample in the batch based on its corresponding sigma
    ...
    def index_for_timestep_bs(self, timesteps, schedule_timesteps=None):
        if schedule_timesteps is None:
            schedule_timesteps = self.timesteps

        # Find indices for each timestep in the batch
        indices = [(schedule_timesteps == timestep).nonzero() for timestep in timesteps]

        # For each timestep, apply the logic for selecting the position (either 1 or 0)
        result_indices = []
        for index_set in indices:
            pos = 1 if len(index_set) > 1 else 0
            result_indices.append(index_set[pos].item())

        # Return the list of indices
        return torch.tensor(result_indices, device=self.sigmas.device)
    ...
    def _init_step_index_bs(self, timesteps):
        if self.begin_index is None:
            if isinstance(timesteps, torch.Tensor):
                timesteps = timesteps.to(self.timesteps.device)

            # Use the modified index_for_timestep function to handle multiple timesteps
            self._step_index = self.index_for_timestep_bs(timesteps)
        else:
            # If begin_index is set, apply it for all timesteps
            self._step_index = torch.full((len(timesteps),), self.begin_index, device=self.sigmas.device)
    ...
    def step_bs(
        ...
        timesteps: Union[List[float], torch.Tensor],  # Change to handle batches
        sample: torch.Tensor,
        s_churn: float = 0.0,
        s_tmin: float = 0.0,
        ...
        if isinstance(timesteps, (int, torch.IntTensor, torch.LongTensor)):
            raise ValueError(
        ...
        # Ensure timesteps is a tensor and on the same device as the sample
        if isinstance(timesteps, list):
            timesteps = torch.tensor(timesteps, device=sample.device)

        # Initialize the step index for the batch of timesteps
        if self.step_index is None:
            self._init_step_index_bs(timesteps)
        ...
        # Retrieve sigmas for each timestep in the batch
        sigmas = self.sigmas[self.step_index]  # Assumes step_index now provides sigmas for all timesteps in the batch

        # Compute gamma for each sigma in the batch
        gamma = torch.where(
            (sigmas >= s_tmin) & (sigmas <= s_tmax),
            s_churn / (len(self.sigmas) - 1),
            torch.tensor(0.0, device=sigmas.device)
        )
        gamma = torch.clamp(gamma, max=(2**0.5 - 1))
        ...
        # Update sample based on gamma
                
        for i in range(gamma.shape[0]):
            if gamma[i] > 0:
                # Apply the noise adjustment only for this specific `i` where gamma is positive
                sample[i] = sample[i] + eps[i] * (sigma_hat[i]**2 - sigmas[i]**2).sqrt()

        # Compute predicted original sample
        if self.config.prediction_type == "original_sample" or self.config.prediction_type == "sample":
            pred_original_sample = model_output
        elif self.config.prediction_type == "epsilon":
            pred_original_sample = sample - sigma_hat.view(-1, 1, 1, 1) * model_output  # Broadcast sigma_hat
        elif self.config.prediction_type == "v_prediction":
            pred_original_sample = model_output * (-sigmas / (sigmas**2 + 1).sqrt()) + (sample / (sigmas**2 + 1))
        else:
            raise ValueError(
        ...
        derivative = (sample - pred_original_sample) / sigma_hat.view(-1, 1, 1, 1)  # Broadcast for consistency

        dt = self.sigmas[self.step_index + 1].view(-1, 1, 1, 1) - sigma_hat.view(-1, 1, 1, 1)  # Broadcast for consistency
